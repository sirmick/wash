...
| Protocol gaps — Xwayland `-auth` (H6) | 🚫 Won't fix (decided 2026-07-03) | Inherited wlroots limitation (shared with sway): wlroots hands Xwayland pre-opened socket fds and passes no `-auth`, and one is an abstract/netns-global socket, so a compositor flag can't gate cross-uid access. NOT an Xwayland bug (it provides `-auth`; GNOME/KDE use it). Only exploitable under shared-host-shared-netns multi-user; a compositor patch can't close it. Mitigable wash-side via per-user netns isolation at the spawn layer IF that deployment materializes — not built. |
