...
/* wash bump: upstream is 16. TinyEMU+OpenSBI inits fill it (~3
   firmware/SU + HTIF console memranges) before IPI MSWI tries to
   add its region, causing ipi init to fail with -3 (EINVAL via
   the count overflow check). 64 is comfortably above the peak. */
#define ROOT_REGION_MAX	64
