...
func Tools() []Tool {
	...
	return []Tool{
		...
		{"member_spawn", "Start a resident or ephemeral teammate with fresh context. Supply role instructions and relevant files. Returns a member ID for messaging.", schema(map[string]any{"name": str(), "provider": str(), "cwd": str(), "instructions": str(), "lifetime": map[string]any{"type": "string", "enum": []string{"resident", "ephemeral"}}, "task": str(), "configs": map[string]any{"type": "object", "additionalProperties": field("string")}, "can_spawn": field("boolean")}, "name", "instructions", "lifetime")},
