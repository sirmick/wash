...
func Tools() []Tool {
	...
	return []Tool{
		...
		{"member_control", "Pause, resume or end members by ID/key, or a package (orchestrator). Members may pause/resume themselves; ending is orchestrator-only. Resume retries a failed launch. Returns per-member outcomes.", schema(map[string]any{"action": enum("pause", "resume", "end"), "member_ids": strings, "package": str}, "action")},
		{"member_update", "Atomically set your status, acknowledge messages, complete/fail assignments and update QA. waiting returns at once: then END YOUR TURN. waiting.until_assignments (ones you created) holds their results for one wake-up when the last resolves. A result may cc members, who get a non-waking copy. QA replies append; transitions need expected_revision; resolving needs evidence and orchestrator/package-reviewer authority.", schema(map[string]any{"status": str, "emoji": str, "waiting": schema(map[string]any{"reason": str, "reply_to": str, "until_assignments": strings}, "reason"), "acknowledge": strings, "assignment_results": array(assignment), "qa_updates": array(qa), "request_id": str})},
		{"message_send", "Send one message, or several atomically as messages. Recipients by member ID or key. instruction/question/answer wake idle members; progress does not. qa:{action:open,id,package,title} opens a thread (assignee defaults to recipient); later replies carry thread_id.", schema(messageInput)},
		{"inbox_read", "Read your retained inbox; does not acknowledge. after is a message ID; limit 1–100.", schema(map[string]any{"after": str, "limit": integer})},
		{"inbox_ack", "Acknowledge received message IDs; does not complete assignments.", schema(map[string]any{"ids": strings, "request_id": str}, "ids")},
