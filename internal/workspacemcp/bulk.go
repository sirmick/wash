...
func Tools() []Tool {
	...
	return []Tool{
		{"workspace_get", "Read the workspace; view=about for operating instructions before setup. The default view=team is the compact who-is-doing-what: per member state, activity, status, open assignments, undelivered mail and current settings. view=state is the full JSON: configuration, profiles, plan, assignments, pending decisions and each live session's config_options (include_messages pages history). view=qa returns generated QA Markdown and threads; thread_id selects paginated events (after event ID, limit 1–100). State omits QA event bodies. package filters QA. No mutation or workspace creation.", schema(map[string]any{"view": enum("state", "about", "qa", "team"), "include_messages": boolean, "after": str, "limit": integer, "thread_id": str, "package": str})},
		{"workspace_configure", "Set up or atomically patch a workspace. Initial setup requires workspace.name; project_root defaults to cwd. Omitted fields stay; profiles replace by alias (null deletes), packages:{CODE:{title}} names each package code for people (null deletes) so the sidebar groups members under it and member names can be just the role, plan.items patch by key (null removes), document:null clears. qa_document.path creates or resumes a Markdown QA file with lossless checkpoints; ordinary Markdown is preserved (null detaches); inspect qa_document_status for write errors. Members are keyed launch definitions: retries reuse; existing definitions cannot silently change. preview validates without mutation/launch. expected_revision guards; request_id deduplicates. Configuration commits before separate launch outcomes; resume failed launches explicitly. Profiles/members accept capability:reviewer for provider-enforced read/search plus scoped coordination; inspect about for supported adapters. Unsupported adapters fail launch. Package implementers/reviewers should be resident until accepted.", schema(map[string]any{"workspace": schema(map[string]any{"name": str, "project_root": str}, "name"), "name": str, "max_active": integer, "max_members": integer, "profiles": map[string]any{"type": "object", "additionalProperties": nullable(profile)}, "default_profile": str, "packages": map[string]any{"type": "object", "additionalProperties": nullable(schema(map[string]any{"title": str}, "title"))}, "members": map[string]any{"type": "object", "additionalProperties": member}, "plan": schema(map[string]any{"items": map[string]any{"type": "object", "additionalProperties": nullable(item)}, "order": map[string]any{"type": "array", "items": str, "maxItems": 500}}), "qa_document": nullable(schema(map[string]any{"path": str, "title": str}, "path")), "document": nullable(schema(map[string]any{"path": str, "title": str}, "path")), "expected_revision": integer, "request_id": str, "preview": boolean})},
		{"workspace_end", "End the workspace's child sessions; preserve the owning conversation, files and history. Final QA save status is returned; failures keep retrying after end. Orchestrator only.", schema(nil)},
		...
		{"member_update", "Atomically report your status, acknowledge messages, complete/fail assignments and update QA. QA replies append; transitions require expected_revision. Resolution needs evidence and orchestrator/package-reviewer authority. waiting returns immediately: END YOUR TURN, do not poll. waiting.until_assignments names assignments you created: their results are held and arrive together in one turn when the last resolves. A complete/fail result may cc members (for example a reviewer cc'ing the implementer); they get a non-waking copy. request_id makes retries safe.", schema(map[string]any{"status": str, "emoji": str, "waiting": schema(map[string]any{"reason": str, "reply_to": str, "until_assignments": strings}, "reason"), "acknowledge": strings, "assignment_results": array(assignment), "qa_updates": array(qa), "request_id": str})},
