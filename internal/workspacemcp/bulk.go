...
func Tools() []Tool {
	...
	return []Tool{
		...
		{"workspace_end", "Orchestrator: end the workspace's child sessions, keeping the owning conversation, files and history. Returns the final QA save status; failed saves keep retrying. workspace_id (full ID or a unique prefix of at least 8) ends another workspace whose orchestrator is not running in Wash, e.g. a stale one still holding a QA file.", schema(map[string]any{"workspace_id": str})},
		...
		{"message_send", "Send one message, or several atomically as messages. Recipients by member ID or key; a member's messages to the orchestrator are at most 2000 bytes. instruction/question/answer wake idle members; progress does not, except that your latest progress to the orchestrator is delivered when you then set waiting. qa:{action:open,id,package,title} opens a thread (assignee defaults to recipient); later replies carry thread_id.", schema(messageInput)},
