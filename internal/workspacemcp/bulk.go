...
func Tools() []Tool {
	...
	return []Tool{
		{"workspace_get", "Read state, or view=about for operating instructions before setup. view=team is the compact who-is-doing-what: per member state, activity, status, open assignments, undelivered mail and current settings. view=qa returns generated QA Markdown and threads; thread_id selects paginated events (after event ID, limit 1–100). State omits QA event bodies. package filters QA. No mutation or workspace creation.", schema(map[string]any{"view": enum("state", "about", "qa", "team"), "include_messages": boolean, "after": str, "limit": integer, "thread_id": str, "package": str})},
