...
// ringBuf.Write must report len(p) consumed even after the buffer is
// full — it never rejects input, just drops the oldest bytes. A wrong
// count makes io.MultiWriter (how spawn wires stdout/stderr into the
// crash log) raise ErrShortWrite once the ring fills.
