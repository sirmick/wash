...
#
# Env:
#   WASH_PKG_VERSION    default: the root VERSION file (the master)
...
# Default to the master VERSION file — the same source the Makefile stamps into
# the binaries — so the package version can't drift from the binary version.
# `make check-versions` guards that the changelog/spec/APKBUILD literals match.
VERSION="${WASH_PKG_VERSION:-$(cat "$ROOT/VERSION")}"
